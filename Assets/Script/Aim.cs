...
public class Aim : MonoBehaviour //挂枪口Muzzle上
{
    ...
    void AimLaunch() //瞄准发射
    {
        if (Input.GetMouseButtonDown(0)) //点击鼠标左键
        {
            ...
            line.active = true;
            line.transform.rotation = Quaternion.Euler(0, 0, Vector3.Angle (line.transform.position, transform.position));
        }
        if (Input.GetMouseButton(0)) //按住鼠标左键不放
        {
            ...
            // Vector3 targetDir = transform.position-line.transform.position;
            Vector3 targetDir = new Vector3(v.x, v.y,0) - line.transform.position;
            float angle =0;
            if(v.x-line.transform.position.x>=0){
                angle = 180-Vector3.Angle(targetDir,new Vector3(0,1,0));
            }else{
                angle = Vector3.Angle(targetDir,new Vector3(0,1,0))-180;
            }
